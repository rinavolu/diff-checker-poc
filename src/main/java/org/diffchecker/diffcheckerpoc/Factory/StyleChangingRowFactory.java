...
import org.diffchecker.diffcheckerpoc.controllers.database.KeyDiff;
...
public class StyleChangingRowFactory<T> implements
    ...
    private final SimpleBooleanProperty is_key_diffs_added;
    ...
    public StyleChangingRowFactory(String styleClass, Callback<TableView<T>, TableRow<T>> baseFactory) {
        ...
        this.is_key_diffs_added = new SimpleBooleanProperty(false);
    }
    ...
    public TableRow<T> call(TableView<T> tableView) {
        ...
        is_key_diffs_added.addListener(new ChangeListener<Boolean>() {

            @Override
            public void changed(ObservableValue<? extends Boolean> observableValue, Boolean aBoolean, Boolean t1) {
                //System.out.println("Value Changed ");
                //TODO add true false check
                updateStyleClassBasedOnKeyDiff(row);
            }
        });
    ...
    public void setKeyDiffs(Map<String, KeyDiff> key_diff_map) {
        is_key_diffs_added.setValue(true);
        this.key_diff_map = key_diff_map;
    }
    ...
    private void updateStyleClassBasedOnKeyDiff(TableRow<T> row) {
        if(row==null) {
        ...
        //System.out.println(row.getTableView().getItems().get(row.getIndex()));
    }
